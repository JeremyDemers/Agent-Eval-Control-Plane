...
The default image is `ghcr.io/jeremydemers/agent-eval-control-plane:0.54.0`. Tagged releases publish
...
`deploy/overlays/gcp-kms` provides Google Cloud KMS and Cloud HSM signing through direct Workload
Identity Federation for GKE. It removes the private-key map, pins one CryptoKeyVersion and protection
level, and uses a dedicated Kubernetes ServiceAccount without a service-account key file. Bind the
included one-permission custom role to that GKE principal on the target CryptoKey. See
